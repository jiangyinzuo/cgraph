...
LSP 生产代码中的 `settings`、`capabilities`、`framing`、`actor`、`documents` 和 `progress` 是职责模块，而协议集成测试仍从父模块驱动真实 actor。这样测试继续覆盖模块拼装后的请求顺序、取消、反向请求和文档生命周期，不把重构后的内部函数调用当成用户行为替代品。
