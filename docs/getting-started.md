...
- 按 `g<` 可查看 LSP 初始化摘要、空 workspace-symbol 查询统计和实际 stderr 日志路径；默认日志为 `/tmp/cgraph-<server>-<pid>.log`。

1. 输入 `at` 打开 type 搜索框。
2. 弹窗包含 LSP Query、Symbol、URI 三个输入框；默认焦点位于 LSP Query，按 `Tab` 依次循环切换。
3. 在 LSP Query 中直接输入任意长度的文本，例如 `V`、`Ve` 或 `Vector builder`；不要求先输入两个字符。停止输入约 200 ms 后，cgraph 会把该输入框的完整文本交给当前分析 provider；继续编辑这一栏会取消过时查询并重新计时。
4. Symbol 和 URI 只对当前 provider 候选执行即时本地模糊筛选，不发送新请求。每栏可用空格提高输入可读性，例如 Symbol 中的 `prs thrd` 会作为忽略空白后的单一模糊序列 `prsthrd` 匹配完整符号名，而不是多个 AND 条件。
5. 防抖期间会显示 `Waiting for typing pause…`；请求真正发出后变为 `Searching workspace symbols…`，完成后显示 provider 候选数和本地匹配数。
6. 使用上下方向键选择结果，再按回车把该类型作为 anchor 加入画布。
7. 重复搜索其他符号时，多个 anchor 会同时显示；重新选择已有语义节点会复用它并移回中心，而不重复创建节点。
8. 使用方向键或 `h`、`j`、`k`、`l` 在可见节点间移动，也可以鼠标单击节点；首次按 `tl` / `tr` 会异步加载对应方向的一层关系，成功后展开并缓存，后续收起再展开不重复请求。
9. 当展开内容超出终端时，按住鼠标左键拖拽节点主体或画布空白处；整个画布会随指针平移，屏幕外节点会进入可见区域。
10. 当前节点是 anchor 时可用 `dd` 取消入口；`dp` / `dn` 清除当前节点左/右分支，按 `q` 退出。普通共享节点执行 `dd` 不会删除整个连通图。
11. 快捷键记不清时按 `?` 打开完整帮助；左下 footer 只保留高频入口。
...
如果 LSP 没有启动，cgraph 会尝试初始化 Rust、C、C++ 或 Python 的 Tree-sitter grammar/query，并在底部状态栏显示结果。LSP 启动失败的详情会先作为普通消息保留；只要 Tree-sitter 后备成功，当前状态就是 `Tree-sitter: <language> · Ready`，不会残留错误状态。只有所有分析 provider 都不可用时才显示最终 Error。LSP 已连接但没有声明当前 hierarchy kind 时，也会透明使用同语言 Tree-sitter 后备；例如 rust-analyzer 当前没有标准 type hierarchy，所以 Rust 类型展开走静态索引，而搜索和调用展开仍走 rust-analyzer。第一次 `ac` / `at` 搜索、纯静态展开或能力回退会惰性扫描项目源文件，之后复用同一索引。Tree-sitter 只返回能按名称唯一绑定的项目内语法关系；动态分派、同名歧义和项目外目标可能省略，展开完成后底栏会显示 `syntactic relations only` 提醒。
