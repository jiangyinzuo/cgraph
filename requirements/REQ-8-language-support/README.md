...
rust-analyzer、clangd 和 Pyrefly 的启动检测、通用 workspace symbol 搜索以及标准 LSP call/type hierarchy 客户端已经实现。Pyrefly 的 `lsp` 子命令由 cgraph 自动补充，方法名使用 `Class.method`；用户仍可用 `--lsp pylsp` 或其他程序覆盖默认选择。各 server 是否实际支持对应标准方法会以成功、空结果或明确错误呈现。
