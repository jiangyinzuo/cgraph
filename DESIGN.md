...
cgraph 是一个基于 LSP 或 Tree-sitter 的交互式 TUI，用于发现符号并浏览函数调用层次和类型继承层次。用户应能从命令行快速进入当前项目的关系画布，在不离开终端的情况下搜索、展开、导航和管理有向关系图。
...
| [REQ-2 分析后端状态](requirements/REQ-2-analysis-status/README.md) | LSP/Tree-sitter 状态和底部同栏展示 |
| [REQ-3 层次关系探索](requirements/REQ-3-hierarchy/README.md) | 展开、缓存、重复节点和刷新 |
| [REQ-4 画布与导航](requirements/REQ-4-canvas-navigation/README.md) | 节点选择、空间导航、viewport 和操作帮助 |
| [REQ-5 符号与图入口管理](requirements/REQ-5-symbol-management/README.md) | `ac`/`at` 搜索、新增、重定位和删除 |
...
| [REQ-9 项目本地配置与符号过滤](requirements/REQ-9-project-configuration/README.md) | `.cgraph.toml`、限定名过滤和会话内编辑重载 |
...
当前实现使用 Rust、Ratatui、Tokio、LSP 协议类型和 Tree-sitter provider。模块职责、数据流和依赖方向见 [`src/README.md`](src/README.md)。rust-analyzer 的独立进程模型、冷索引原因和未来复用方案见 [`src/fetch/rust-analyzer.md`](src/fetch/rust-analyzer.md)。
